...
public:
    ...
    void create(id_type const &id, std::vector<id_type> const &parent_ids) {
        ...
        addEdges(newNode, parent_ids, guards); //checks if parents nodes exist (throws if not)
...
private:
    ...
    void addEdges(std::shared_ptr<Node> &newNode, std::vector<id_type> const &parent_ids,
                  std::vector<ScopeGuard> &guards) {
